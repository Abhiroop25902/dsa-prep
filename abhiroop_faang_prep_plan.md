...
*Last updated: 2026-07-11 (Phase 1, 5 topics confirmed — Linked List next)*
...
## Currently Assigned
**Phase 1 progress:** Arrays & Hashing, Two Pointers, Sliding Window, Stack, and Binary Search all confirmed/closed. Moving to **Linked List** per the Phase 1 roadmap order.
...
**2026-07-11 result:** Three problems solved in one sitting — LC 424 (47 min, Sliding Window), LC 20 (9 min, Stack), LC 704 (8 min, Binary Search). All self-debugged, all sections confirmed and closed. See Progress Tracker for detail.
...
|------|-------|-------|-------------------|------|-------------------|-------|
| 2026-07-11 | Phase 1 | Sliding Window / Stack / Binary Search | LC 424 — Longest Repeating Character Replacement (47 min, 12:15-1:02 PM, 3 self-debugged bugs, confidence 3/5); LC 20 — Valid Parentheses (9 min, 2:08-2:17 PM, 1 self-debugged bug, confidence 4/5); LC 704 — Binary Search (8 min, 2:25-2:33 PM, 1 self-debugged bug, confidence 5/5) | ~64 min total | 3/5, 4/5, 5/5 | Three problems, one sitting. **LC 424:** Correct sliding window + frequency-count approach derived independently. Key insight: swapsRequired = windowLength - maxFrequency. Bugs: wrong right increment condition, missing initial char save into map, off-by-one in array size — all self-diagnosed. TC O(n*26), SC O(1). Runtime 19th percentile — bottleneck is rescanning full 26-element array inside shrink loop. **LC 20:** Clean stack implementation using ArrayList. Researched Stack vs ArrayList first — correctly identified thread-safety overhead as reason to avoid java.util.Stack. One bug: forgot empty-stack check before accessing top. Runtime 87th percentile. **LC 704:** Correctly recalled overflow fix (left + (right-left)/2), noted compiler optimizes /2 to bit-shift. Made insightful observation that binary search is "two pointers that converge." One typo bug (mid vs nums[mid]). Runtime 100th percentile. All three sections confirmed and closed. |
...
- Total problems solved since restart: 14 (LC300/208/215/684/207/210/238/49/11/15/3/424/20/704)
- Active days so far: 8 of 9 calendar days (2026-07-03 to 07-11; 07-07 was a gap)
- Current streak: 4 days (07-08, 07-09, 07-10, 07-11)
- Current phase: Phase 1 — Pattern Reactivation. 5 sections confirmed/closed (Arrays & Hashing, Two Pointers, Sliding Window, Stack, Binary Search); Linked List is next.
