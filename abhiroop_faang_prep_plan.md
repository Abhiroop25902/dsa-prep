...
**Next session:**
- Problem: [LC 78 — Subsets (Medium)](https://leetcode.com/problems/subsets/)
- Why: same backtracking pattern as LC 46, but collect at every node not just leaves — reinforces the template with a twist.
- Language: Java, timed 15-20 min, no hints
- Report back: time taken, approach, any issues

**2026-07-18 result:** LC 226 — Invert Binary Tree (2 min, DFS recursive swap, 0 bugs, 100th percentile). Trivial confirm, trees section closed (4 problems: LC104, LC102, LC98, LC226). LC 46 — Permutations (20 min, backtracking with HashSet + Set.copyOf, 0 bugs, accepted first submit). Correct approach, no hints, but Set.copyOf at every level adds unnecessary O(n²) space — standard `boolean[] used` pattern is cleaner and what interviewers expect. Confidence 3/5.
...
|------|-------|-------|-------------------|------|-------------------|-------|
| 2026-07-18 | Phase 1 | Trees / Backtracking | LC 226 — Invert Binary Tree (Easy, 2 min, 0 bugs, 100th percentile); LC 46 — Permutations (Medium, 20 min, 0 bugs, accepted) | ~22 min total | 5/5, 3/5 | Two problems. **LC 226:** DFS recursive swap, trivial confirm — trees section closed (4 problems total). **LC 46:** Backtracking with HashSet + Set.copyOf for remaining-elements tracking. Correct approach, no hints, but full time budget used. Set.copyOf at every recursion level inflates space to O(n²) — standard `boolean[] used` array pattern is O(n) space and cleaner. Runtime 6ms (2.31%), Space 46.8MB (5.54%). Next: LC 78 (Subsets) to reinforce backtracking template. |
...
- Total problems solved since restart: 21 (LC300/208/215/684/207/210/238/49/11/15/3/424/20/704/206/2/104/102/98/226/46)
